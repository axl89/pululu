...
func (mod *modContext) genMethods(w io.Writer, res *schema.Resource) {
	...
	genMethod := func(method *schema.Method) {
		...
		if fun.Inputs != nil {
			...
			args = make([]*schema.Property, 0, len(fun.Inputs.Properties)-1)
			for _, arg := range fun.Inputs.Properties {
		...
		for _, arg := range args {
			...
			ty := mod.typeString(arg.Type, true, true /*acceptMapping*/)
			fmt.Fprintf(w, ",\n%s%s: %s = None", indent, pname, ty)
		}
